...
public class CommConnection {
	...
	//private List<CommListener> listeners = new ArrayList<CommListener>();
	...
	private void init() {
		...
		try {
			...
			CommunicationInitializer ci = new CommunicationInitializer(handler,false);
...
/*			
			if (listeners.size() > 0) {
				for (CommListener cl : listeners)
					handler.addListener(cl);
				listeners.clear();
			}
