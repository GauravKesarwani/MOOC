...
public class ResourceUtil {
	 ...
	 * @return
	 */
	public static Header buildHeaderFrom(Header reqHeader, PokeStatus status,
			String statusMsg) {
		return buildHeader(reqHeader.getRoutingId(), status, statusMsg,
				reqHeader.getOriginator(), reqHeader.getTag());
	}

	public static Header buildHeader(Routing path, PokeStatus status,
	...
	public static Request buildError(Request request, PokeStatus status,
			String statusMsg) {
		Request.Builder bldr = Request.newBuilder();
		Header hdr = buildHeaderFrom(request.getHeader(), status, statusMsg);
		bldr.setHeader(hdr);

		Payload.Builder pb = Payload.newBuilder();
		bldr.setBody(pb.build());
		return bldr.build();
	}

	public static Header buildHeaderforServer(Header header, String node) {
		Header.Builder bldr = Header.newBuilder();
		bldr.setOriginator(header.getOriginator());
		bldr.setRoutingId(header.getRoutingId());
		bldr.setTag(header.getTag());
		bldr.setToNode(node);
		bldr.setTime(header.getTime());
		return bldr.build();
	}

	public static Header buildHeaderforVoting(Header header, String node,
			String originator) {
		Header.Builder bldr = Header.newBuilder();
		bldr.setRoutingId(header.getRoutingId());
		bldr.setTag(header.getTag());
