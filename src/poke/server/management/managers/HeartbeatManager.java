...
public class HeartbeatManager extends Thread {
	...
	public void setIncomingHB(
			ConcurrentHashMap<String, HeartbeatData> incomingHB) {
		this.incomingHB = incomingHB;
	}

	public void setOutgoingHB(
			ConcurrentHashMap<String, HeartbeatData> incomingHB) {
		this.incomingHB = incomingHB;
	}
	...
	public void addOutgoingChannel(String nodeId, String host, int mgmtport,
		...
		if (!outgoingHB.containsKey(ch)) {
			HeartbeatData heart = new HeartbeatData(nodeId, host, null,
					mgmtport);
			...
			logger.error(
					"Received a HB connection unknown to the server, node ID = ",
					nodeId);
		}
	}
	 ...
	 * @param sa
	 */
	public void addAdjacentNodeChannel(String nodeId, Channel ch,
			SocketAddress sa) {
		HeartbeatData hd = incomingHB.get(nodeId);
		if (hd != null) {
			...
			logger.error("Received a HB ack from an unknown node, node ID = ",
					nodeId);
		}
	}
					...
					for (HeartbeatData hd : outgoingHB.values()) {
						...
						try {
							...
							if (logger.isDebugEnabled())
								logger.debug("beat (" + nodeId + ") sent to "
										+ hd.getNodeId() + " at "
										+ hd.getHost());
						} catch (Exception e) {
							hd.incrementFailuresOnSend();
							logger.error("Failed " + hd.getFailures()
									+ " times to send HB for " + hd.getNodeId()
									+ " at " + hd.getHost(), e);
						}
					}
				} else
	...
	public class CloseHeartListener implements ChannelFutureListener {
		...
		public void operationComplete(ChannelFuture future) throws Exception {
			if (outgoingHB.containsValue(heart)) {
				logger.warn("HB outgoing channel closing for node '"
