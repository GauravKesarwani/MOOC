			...
			try {
				...
				if (req.hasBeat()) {
					...
					HeartbeatManager.getInstance()
							.processRequest(req.getBeat());
				} else if (req.hasElection()) {
					ElectionManager.getInstance().processRequest(
							req.getElection());
				} else if (req.hasGraph()) {
					NetworkManager.getInstance().processRequest(req.getGraph(),
							msg.channel, msg.sa);
				} else if (req.hasJobBid()) {
					JobManager.getInstance().processRequest(req.getJobBid());
				} else if (req.hasJobPropose()) {
					JobManager.getInstance()
							.processRequest(req.getJobPropose());
				} else
