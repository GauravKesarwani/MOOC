...
import poke.server.conf.NetworkNodeDesc;
...
public class Server {
	...
	protected static HeartbeatManager heartbeatMgr;
	protected static ElectionManager electionMgr;
	private static HashMap<String, NetworkNode> hm = new HashMap<String, NetworkNode>();
	private static HeartbeatConnector serverhbconn = null;

	/*
	 * protected variable Election Co-ordinator. Election Co-ordinator will be a
	 * static variable for all nodes. It will start the election when all the
	 * nodes are up In the case of node failure it will manage the election
	 * between existing running nodes accordingly
	 */

	// protected static ElectionCoordinator electionCoord;
	/**
	...
	public static HashMap<String, NetworkNode> getNetwork() {
		return hm;
	}
	...
	public static ElectionManager getElectionMgr() {
		return electionMgr;
	}

	public static HeartbeatConnector getserverhbconn() {
		return serverhbconn;
	}

	public static HeartbeatManager gethbManager() {
		return heartbeatMgr;
	}
	...
	private void startManagers() {
		...
		for (NetworkNodeDesc nnd : conf.getNetworkStructure().getNodes()
				.values()) {
			NetworkNode netnode = new NetworkNode(nnd.getNodeId(),
					nnd.getHost(), nnd.getPort(), nnd.getMgmtPort());
			hm.put(nnd.getNodeId(), netnode);

		}

		// Pass the HeartbeatManager object to retrieve the nearest nodes
		// information
		electionMgr.setHeartbeatManager(heartbeatMgr);
		electionMgr.setHeartbeatConnector(conn);
