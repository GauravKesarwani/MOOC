				...
				try {
					...
					if (conn.isWritable()) {
						...
						if (channel != null && channel.isOpen()
								&& channel.isWritable()) {
							logger.info("Message for client is/ outbound msg : "+msg);
			...
			while (true) {
				...
				try {
					...
					if (msg instanceof Request) {
						...
						//if request is for voting, then acquire the vote casted by the node
						if(req.getHeader().getOriginator().equalsIgnoreCase("server")
				&& req.getBody().hasInitVoting()) {
							req= ResourceFactory.getInstance().getRequest();
						}
										
						Request reply = null;
						if (rsc == null) {
							logger.error("Either the message has been sent to another node or "
									+ "failed to obtain resource for "
									+ req);
							reply = ResourceUtil.buildError(req,
